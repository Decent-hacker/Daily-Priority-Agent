...
def build_plan(tasks: List[Task]) -> Dict:
    ...
    def pick_within_time(items, limit):
        picked = []
        total = 0
        for (t, s, b) in items:
            if total + t.effort_min <= limit:
                picked.append((t, s, b))
                total += t.effort_min
        return picked

    unblocked_all = [(t, s, b) for (t, s, b) in scored if not t.blocked]
    blocked = [(t, s, b) for (t, s, b) in scored if t.blocked]

    unblocked = pick_within_time(unblocked_all, AVAILABLE_MIN)
